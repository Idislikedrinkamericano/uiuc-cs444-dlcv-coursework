...
from finetune import ViTLinear, inference, Trainer, VPTDeep  # 修改这里
import yaml
...
FLAGS = flags.FLAGS
flags.DEFINE_string('exp_name', 'vpt_deep',  # 修改默认值
...
flags.DEFINE_string('data_dir', './flower-dataset-reduced', 'Directory with flower data')
...
def get_config(exp_name):
    ...
    # 修改这里的 encoder_registry
    encoder_registry = {
        'ViTLinear': ViTLinear,
        'VPTDeep': VPTDeep,  # 使用 VPTDeep
    }
...
def main(_):
    ...
    # 创建模型 - VPTDeep 需要 num_prompts 参数
    model = net_class(102, encoder_name='vit_b_32', num_prompts=20)
    model.to(device)
