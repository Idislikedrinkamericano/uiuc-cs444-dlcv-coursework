...
class ScoreNet(nn.Module):
    ...
    def perturb(self, batch):
        ...
        idx = torch.randint(0, len(self.sigmas), (batch_size,), device=device)
        used_sigmas = self.sigmas[idx].view(-1, 1)
    ...
    def sample(self, batch_size, img_size, sigmas=None, n_steps_each=10, step_lr=2e-5):
        ...
        :return: image trajectories (sigmas.size(0), n_steps_each, N, D)
        """
        self.eval()
        if sigmas is None:
            sigmas = self.sigmas
        ...
        traj = []
        if len(sigmas) == 0 or n_steps_each <= 0:
            return x.unsqueeze(0).unsqueeze(0).expand(len(sigmas), n_steps_each, -1, -1)
    ...
    def get_loss(self, x):
        ...
        score = self.get_score(x_tilde, sigma)
        target = -(noise) / (sigma ** 2)

        loss = 0.5 * ((score - target) ** 2 * (sigma ** 2)).mean()
